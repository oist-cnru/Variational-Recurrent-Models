...
def test_performance(agent_test, env_test, action_filter, times=5):
    ...
    for _ in range(times):
        ...
        for _ in range(max_steps):
            ...
            sp_seq[-1, -1] = r
            a = agent_test.select(sp_seq, action_return='normal')  # use tanh(mu_a) for evaluating performance
            EpiTestRet += r
            if done:
                break
...
V_real = np.zeros([max_episodes, max_steps], dtype=np.float32)  # whether a step is valid value: 1 (compute gradient at this step) or 0 (stop gradient at this step)
    ...
    for t in range(max_steps):
        ...
        if pre_train and global_step == step_start_st + 1:
            for _ in range(5000):
                ...
                sample_es = np.random.choice(e_real, batch_size, p=weights / weights.sum())

                SP = S_real[sample_es, 1:].reshape([batch_size, -1, env.observation_space.shape[0]])
                A = A_real[sample_es].reshape([batch_size, -1, env.action_space.shape[0]])
        ...
        if global_step > step_start_rl and np.random.rand() < train_freq_rl:
            ...
            for _ in range(max(1, int(train_freq_rl))):
                ...
                SP = S_real[sample_es, 1:].reshape([batch_size, -1, env.observation_space.shape[0]])
                S0 = S_real[sample_es, 0].reshape([batch_size, env.observation_space.shape[0]])
                A = A_real[sample_es].reshape([batch_size, -1, env.action_space.shape[0]])
                R = R_real[sample_es].reshape([batch_size, -1, 1])
                ...
                agent.train_rl_sac(x_obs=np.concatenate((SP, R), axis=-1), s_0=S0, 
                                   a_obs=A, r_obs=R, d_obs=D, validity=V, gamma=0.99, equal_pad=equal_pad)

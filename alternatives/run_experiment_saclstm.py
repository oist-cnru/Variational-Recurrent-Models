...
def test_performance(agent_test, env_test, action_filter, times=5):
    ...
    for _ in range(times):
        ...
        for _ in range(max_steps):
            ...
            a = agent_test.select(sp, action_return='normal')  # use tanh(mu_a) for evaluating performance
            EpiTestRet += r
            if done:
                break
    ...
    for _ in range(times):
        ...
        for _ in range(max_steps):
            if np.any(np.isnan(a)):
                raise ValueError
            sp, r, done, _ = env_test.step(action_filter(a))
            a = agent_test.select(sp, action_return='mean')  # use tanh(mu_a) for evaluating performance
            EpiTestRet_mean += r
            if done:
                break
...
agent = SACRNN(env.observation_space.shape[0], env.action_space.shape[0], gamma=0.99, reward_scale=reward_scale, beta_h=beta_h)
agent_test = SACRNN(env.observation_space.shape[0], env.action_space.shape[0], gamma=0.99, reward_scale=reward_scale, beta_h=beta_h)

# SP_real = np.zeros([max_episodes, max_steps, env.observation_space.shape[0]], dtype=np.float32)
S_real = np.zeros([max_episodes, max_steps + 1, env.observation_space.shape[0]], dtype=np.float32)
A_real = np.zeros([max_episodes, max_steps, env.action_space.shape[0]], dtype=np.float32)
...
V_real = np.zeros([max_episodes, max_steps],
                  dtype=np.float32)  # whether a step is value: 1 (compute gradient at this step) or 0 (stop gradient at this step)
    ...
    for t in range(max_steps):
        ...
        if global_step == step_start_rl + 1:  # pretrain
            for _ in range(5000):
                ...
                sample_es = np.random.choice(e_real, minibatch_size, p= weights / weights.sum())

                S = S_real[sample_es, :-1, :].reshape([minibatch_size, -1, env.observation_space.shape[0]])
                SP = S_real[sample_es, 1:, :].reshape([minibatch_size, -1, env.observation_space.shape[0]])
                A = A_real[sample_es].reshape([minibatch_size, -1, env.action_space.shape[0]])
                ...
                agent.learn(S, SP, R, A, D, V, computation=computation_mode, grad_clip=grad_clip, seq_len=seq_len)

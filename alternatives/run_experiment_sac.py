...
agent = SAC(env.observation_space.shape[0], env.action_space.shape[0], gamma=0.99, reward_scale=reward_scale, beta_h=beta_h)
agent_test = SAC(env.observation_space.shape[0], env.action_space.shape[0], gamma=0.99, reward_scale=reward_scale, beta_h=beta_h)
...
while global_step < max_all_steps:
    ...
    for t in range(max_steps):
        ...
        if global_step > step_start_rl and np.random.rand() < train_freq_rl:
            ...
            for _ in range(max(1, int(train_freq_rl))):
                ...
                sample_stp = np.arange(sample_start_step, sample_start_step + batch_size)
                ...
                agent.learn(S, SP, R, A, D, V, computation=computation_mode, grad_clip=grad_clip)
    ...
    print(env_name + " -- episode {} : steps {}, mean reward {}".format(e_real, t, np.mean(R_all[0, global_step - t: global_step])))
    e_real += 1
